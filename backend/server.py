...
# Duplicate app creation - commented out
# app = FastAPI(title="ListingSpark AI", version="2.0.0")

# First CORS middleware removed (duplicate)
...
async def register(user_data: UserCreate):
    ...
    try:
        logger.info(f"Register attempt - Email: {user_data.email}, Name: {user_data.full_name}")
...
async def get_mls_accounts(
    ...
    """Get MLS accounts from SQLite"""
    try:
        user_id = current_user["id"] if isinstance(current_user, dict) else current_user.id
        accounts = sqlite_db.get_mls_accounts(user_id)
        return [MLSAccount(**{k: v for k, v in acc.items() if k not in ['client_id', 'client_secret']}) for acc in accounts]
    except Exception as e:
        logger.error(f"Error fetching MLS accounts: {e}")
        return []
...
async def get_dashboard_stats(
    ...
    try:
        user_id = current_user["id"] if isinstance(current_user, dict) else current_user.id
        stats = sqlite_db.get_dashboard_stats(user_id)
        return stats
    except Exception as e:
        logger.error(f"Error fetching dashboard stats: {e}")
        return {"total_listings": 0, "published": 0, "status_counts": {}, "mls_accounts": 0, "recent_listings": []}
