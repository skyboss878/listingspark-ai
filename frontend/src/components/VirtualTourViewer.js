...
const VirtualTourViewer = () => {
  ...
  }, [propertyId, selectedTour, loading]);

  useEffect(() => {
    loadTours();
    const interval = setInterval(loadTours, 5000);
  ...
  const handleTourSelect = async (tour) => {
    ...
    try {
      await api.tours.trackView(tour.id);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 text-white">
      ...
      {selectedTour && selectedTour.processing_status === 'completed' && (
        <div className="w-full border-t border-white/10" style={{ height: 'calc(100vh - 200px)' }}>
          <iframe
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {completedTours.map((tour) => (
            <motion.div
              ...
              className={`glass rounded-xl p-6 cursor-pointer transition-all ${selectedTour?.id === tour.id ? 'ring-2 ring-purple-400' : ''}`}
            >
