...
echo "Fixing ESLint errors in React components..."

# Fix Analytics.js - Move loadAnalytics before useEffect
cat > src/components/Analytics.js << 'ANALYTICS_EOF'
...
# Fix Dashboard.js - Add fetchDashboardData to dependency array
sed -i 's/}, \[user\]);/}, [user, fetchDashboardData]);/' src/components/Dashboard.js

# Fix ViralContentGenerator.js - Move functions before useEffect
sed -i '15,20d' src/components/ViralContentGenerator.js
sed -i '14a\
\
...
\
  useEffect(() => {\
    loadContent();\
  }, [loadContent]);' src/components/ViralContentGenerator.js

# Fix VirtualTourViewer.js - Move loadTours before useEffect
sed -i '15,17d' src/components/VirtualTourViewer.js
sed -i '14a\
\
...
\
  useEffect(() => {\
    loadTours();\
  }, [loadTours]);' src/components/VirtualTourViewer.js

echo "ESLint errors fixed!"
echo "Files modified:"
echo "  - src/components/Analytics.js"
echo "  - src/components/Dashboard.js"
echo "  - src/components/ViralContentGenerator.js"
echo "  - src/components/VirtualTourViewer.js"
