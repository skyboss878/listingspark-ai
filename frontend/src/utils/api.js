...
const api = {
  ...
  users: {
    create: async (email, name) => {
      const response = await axios.post('/auth/register', { email, full_name: name, password: 'temp123' });
  ...
  // Viral Content
  viralContent: {
    generate: async (propertyId, options = {}) => {
      const response = await axios.post(
        `/properties/${propertyId}/viral-content`,
        { voice: options.voice || 'professional_female' }
      );
      return response.data;
    },
